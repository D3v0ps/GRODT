-- ============================================================
-- GRODT – branschklassificering (AI-omdöme, engångskörning)
--
-- SNI-koden ensam har registreringsfel och beskrivningen ensam är
-- suddig. Därför lagras ett sammanvägt omdöme per bolag – satt av en
-- engångskörning via Claude (bransch_klass_kalla = 'ai') utifrån namn +
-- SNI + verksamhetsbeskrivning, eller manuellt på bolagssidan. När
-- omdömet finns är det den auktoritativa signalen för målbilden; saknas
-- det faller vi tillbaka på SNI-regeln.
--
--   arbetsformedling  → inom målbild (säljbar)
--   personaluthyrning → utanför (bemanning/uthyrning)
--   annat             → utanför (helt annan bransch)
-- ============================================================

alter table public.companies
  add column bransch_klass text
    check (bransch_klass in ('arbetsformedling', 'personaluthyrning', 'annat')),
  add column bransch_klass_kalla text,
  add column bransch_klass_at timestamptz;

-- ---------- list_leads v9: bransch_klass + verksamhet i listsvaret ----------
drop function public.list_leads(text, text, text, uuid, boolean, boolean, bigint, bigint, integer, integer, integer, integer, numeric, text, text, integer, integer);
...
as $$
  with base as (
    select
      ...
      l.off_target_sni,
      c.bransch_klass,
      left(c.verksamhetsbeskrivning, 160) as verksamhet,
      l.updated_at
    from public.leads l
