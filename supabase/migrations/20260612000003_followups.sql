-- ============================================================
-- GRODT – uppföljningar (att göra-lista) och förlustorsak
--
-- Ett lead har EN aktiv uppföljning åt gången ("kontakta om 3 mån"):
-- datum, valfri anteckning och vem som ska göra det. När den bockas av
-- nollställs fälten och händelsen loggas – historiken bor i activities.
-- ============================================================

alter table public.leads
  add column follow_up_at date,
  add column follow_up_note text,
  add column follow_up_user uuid references public.profiles (id) on delete set null,
  add column forlust_orsak text;

create index idx_leads_follow_up on public.leads (follow_up_at)
  where follow_up_at is not null;

-- ---------- list_leads v4: uppföljningsdatum i listsvaret ----------
...
revoke execute on function public.list_leads(text, text, text, uuid, bigint, bigint, integer, integer, integer, integer, numeric, text, text, integer, integer) from public, anon;
grant execute on function public.list_leads(text, text, text, uuid, bigint, bigint, integer, integer, integer, integer, numeric, text, text, integer, integer) to authenticated, service_role;
