-- ============================================================
-- GRODT – AF-leverantörsmärkning (pengasäcken)
--
-- Bolag som är godkända leverantörer i Arbetsförmedlingens
-- valfrihetssystem (Rusta och matcha) är upphandlade och betalda av
-- staten – extra heta att jaga. Märkningen sätts av AF-importen och
-- visas som en pengasäck vid bolagsnamnet, med AF:s stjärnbetyg (1–5)
-- i tooltipen.
-- ============================================================

alter table public.companies
  add column af_leverantor_at timestamptz,
  add column af_rating smallint check (af_rating between 1 and 5);

-- ---------- list_leads v10: AF-märkningen i listsvaret ----------
...
as $$
  with base as (
    select
      ...
      (c.af_leverantor_at is not null) as af_leverantor,
      c.af_rating,
      l.updated_at
    from public.leads l
